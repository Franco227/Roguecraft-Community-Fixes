...
scoreboard players operation @e[type=minecraft:marker,tag=master] temp_health -= @e[type=minecraft:marker,tag=master] adrenaline_health
...
scoreboard players set @e[type=minecraft:marker,tag=master,limit=1] temp_health 0
...
tellraw @s[scores={toggle_ability_feedback=0}] ["",{"translate":"roguecraft.chat_messages.use_ability","italic":true,"color":"white","with":[{"translate":"roguecraft.class_ability.glass_cannon"}]},{"text":" (","color":"white","italic":true},{"translate":"roguecraft.generic.strength","italic":true,"color":"white"},{"text":" "},{"score":{"name":"@e[type=minecraft:marker,tag=master,limit=1]","objective":"temp_health"},"color":"white"},{"text":")","italic":true,"color":"white"}]
execute store result score @s current_strength run scoreboard players get @e[type=minecraft:marker,tag=master,limit=1] temp_health