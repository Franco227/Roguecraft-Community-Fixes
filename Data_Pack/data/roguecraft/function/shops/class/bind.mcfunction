...
#bind
$execute unless score @s class matches $(id) unless entity @e[type=minecraft:marker,tag=unbind] run playsound minecraft:block.beacon.power_select master @s ~ ~ ~ 1 1.25
$execute unless score @s class matches $(id) unless entity @e[type=minecraft:marker,tag=unbind] run tellraw @s {"translate":"roguecraft.chat_messages.class_select","italic":true,"color":"green","with":[{"translate":"roguecraft.class.name.$(upgrade)"},{"translate":"roguecraft.chat_messages.class"}]}

#set scoreboard
$execute unless entity @e[type=minecraft:marker,tag=unbind] run scoreboard players set @s class $(id)
