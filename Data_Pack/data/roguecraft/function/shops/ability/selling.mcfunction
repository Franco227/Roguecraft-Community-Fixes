...
$tellraw @s [{"translate":"roguecraft.chat_messages.upgrade_sell","italic":true,"color":"gray","with":[{"translate":"roguecraft.ability.name.$(upgrade)"}]},{"translate":" (%s/%s)","italic":true,"color":"gray","with":[{"score":{"name":"@s","objective":"$(upgrade)"}},{"score":{"name":"@n[distance=..1,type=minecraft:marker]","objective":"level_max"}}]}]
