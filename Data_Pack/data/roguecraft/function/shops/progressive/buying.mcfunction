...
$tellraw @s [{"translate":"roguecraft.chat_messages.upgrade_buy","italic":true,"color":"green","with":[{"translate":"roguecraft.upgrade.name.$(upgrade)"}]},{"translate":" (%s/%s)","italic":true,"color":"green","with":[{"score":{"name":"@s","objective":"$(upgrade)"}},{"score":{"name":"@n[distance=..1,type=minecraft:marker]","objective":"level_max"}}]}]
