scoreboard players set @s parry_timer 0
execute at @s on attacker run damage @s 10 minecraft:generic by @a[sort=nearest,limit=1]
playsound minecraft:block.anvil.land master @a ~ ~ ~ 0.75 1.5
playsound minecraft:entity.wither.break_block master @a ~ ~ ~ 0.75
scoreboard players remove @s mana 5
scoreboard players add @s soul_charge 1
scoreboard players operation @s mana += @e[type=minecraft:marker,tag=master,limit=1] ability_cost_5_1
function roguecraft:ability/tell_1

execute as @e[tag=id,type=minecraft:marker] run function roguecraft:set_mana_bar with entity @s data