...
execute as @e[type=minecraft:item_display,tag=wildfire_head] at @s run tp @s ~ ~ ~ facing entity @a[gamemode=!spectator,sort=nearest,limit=1] eyes
...
execute as @e[type=minecraft:marker,scores={boss_health=1..},tag=wildfire] at @s if entity @a[distance=..128] run forceload add ~ ~

execute as @e[type=minecraft:marker,scores={boss_health=1..},tag=wildfire] at @s run function roguecraft:wildfire/set_shield_hitbox with entity @s data
execute as @e[type=minecraft:marker,scores={boss_health=1..},tag=wildfire] run function roguecraft:wildfire/check_hitboxes with entity @s data

execute as @e[type=minecraft:marker,scores={boss_health=1..},tag=wildfire] run scoreboard players remove @s attack_cooldown 1
