...
$tellraw @s [{"translate":"roguecraft.chat_messages.upgrade_buy","italic":true,"color":"green","with":[{"translate":"roguecraft.ability.name.$(upgrade)"}]},{"translate":" (%s/%s)","italic":true,"color":"green","with":[{"score":{"name":"@s","objective":"$(upgrade)"}},{"score":{"name":"@e[distance=..1,type=minecraft:marker,sort=nearest,limit=1]","objective":"level_max"}}]}]
