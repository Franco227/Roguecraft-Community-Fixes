...
execute as @e[type=minecraft:item_display,sort=nearest,limit=1] at @s run tp @s ~ ~ ~ ~180 ~
$execute positioned ~ ~1.75 ~ unless entity @e[type=minecraft:item_display,tag=lock,distance=..1.5] run summon minecraft:item_display ~ ~ ~ {item:{id:"totem_of_undying",Count:1b,components:{"minecraft:custom_model_data":403}},Rotation:[$(rotation).0f,0.0f],Tags:["lock"]}
$summon minecraft:text_display ~ ~0.6 ~ {text:'[{"translate":"roguecraft.ability.name.$(name)"},{"text":"\\n"}]',Rotation:[$(rotation).0f,0.0f]}
