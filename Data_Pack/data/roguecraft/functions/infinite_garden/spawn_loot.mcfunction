$execute if data entity @s data.$(inventory_slot) run summon item ~ ~ ~ {Tags:[infinite_garden_item],Item:{id:"minecraft:dirt",count:1,components:{}}}
...
$execute if data entity @s data.$(inventory_slot).components run data modify entity @e[tag=infinite_garden_item,tag=!tagged,limit=1] Item.components set from entity @s data.$(inventory_slot).components
