...
execute as @e[type=marker,tag=fireball] run execute store result entity @s data.X double 0.00625 run data get entity @s Pos[0] 6
execute as @e[type=marker,tag=fireball] run execute store result entity @s data.Y double 0.00625 run data get entity @s Pos[1] 6
execute as @e[type=marker,tag=fireball] run execute store result entity @s data.Z double 0.00625 run data get entity @s Pos[2] 6
execute at @s run playsound minecraft:entity.blaze.shoot master @a ~ ~ ~

execute at @s positioned ~ ~1 ~ run summon fireball ^ ^ ^1.5 {ExplosionPower:2,Tags:["player_fireball","current_fireball"]}
