...
execute if score @e[tag=master,type=marker,limit=1] difficulty_level matches ..4 run advancement grant @a[tag=!garden,tag=!hub,advancements={roguecraft:roguecraft/speedrun=false}] only roguecraft:roguecraft/speedrun
execute if score @e[tag=master,type=marker,limit=1] difficulty_level matches ..2 run advancement grant @a[tag=!garden,tag=!hub,advancements={roguecraft:roguecraft/world_record=false}] only roguecraft:roguecraft/world_record
...
execute unless data storage roguecraft:master {bosses_defeated:{tyrannus:1b}} in roguecraft:infinite_garden run summon marker 0 100 0 {Tags:["boss_defeated"]}
